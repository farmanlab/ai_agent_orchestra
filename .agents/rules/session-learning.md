...
ユーザーが承認したら、AskUserQuestionで保存先を確認：

1. **別ブランチで作成（推奨）**: stash → 新ブランチ作成 → コミット＆プッシュ → 復帰
2. **現在のブランチに追加**: 今のブランチに直接追加（コミットはユーザーに任せる）
3. **ユーザーローカルに追加**: `~/.claude/rules/` または `~/.agents/rules/` に保存

詳細な手順は `/review-session` コマンドの Step 6 を参照。
